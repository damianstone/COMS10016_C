...
struct text {
    int capacity; // the total size of the array, not the length 
    char *content; // array of characteres 
};
...
text *newText(char *s) {
    ...
    strcpy(t->content, s); // copy char s into t-content
    return t;
}
...
text *copy(text *t) {
    text *c = malloc(sizeof(text));
    c->content = malloc(t->capacity);
    strcpy(c->content, t->content);
    return c;
}
...
// strstr() => find the first ocurrence of the substring p->content
//  in the string t-> content
